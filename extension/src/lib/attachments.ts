...
// Local mirror of the website's src/lib/file-attachments.ts (kept
// self-contained here rather than cross-imported, same as the rest of the
// extension's popup logic -- see extension/README.md). Gemini reads PDFs,
// images, plain text and JSON natively as inline file parts, so nothing
// extra is needed server-side.
//
// The size cap keeps the base64-inflated request body comfortably under
// typical serverless function body-size limits -- base64 adds ~33%
// overhead, so 4MB raw stays safely under a 4.5MB ceiling even with
